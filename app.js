...
var homeLogController = require('./controllers/homeLogged');
...
app.get('/', homeLogController.index);
app.get('/mydata', function(req, res) {
  res.send('skajf6');
  if(!(typeof req.query.array === 'undefined')){
    var loadedEvents = req.query.array.split(',');
    if (res.user != 'undefined'){
      ...
      for (i=0;i<events.length;i+=2) {
        events.push(
          {
            "url": loadedEvents[i],
            "timeSpent": loadedEvents[i+1],
            "description": "Default description."
          }
        );
      }
 
      req.user.journeys.push(
         {
           "journeyName": "Visited " + loadedEvents[0],
           "timeSpent": loadedEvents[1],
           "description": "Default description",
           "entries": events
         }   
      );
